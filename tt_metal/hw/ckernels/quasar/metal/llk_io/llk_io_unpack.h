...
inline void llk_wait_tiles(const std::int32_t dfb_id, const std::uint32_t num_tiles) {
    ...
    // TT_WAIT_TILES(ckernel::p_stall::STALL_UNPACK, num_tiles, tc_id);        //     uint16_t entries_received;
    uint16_t tiles_received;
    do {
        tiles_received = ckernel::trisc::tile_counters[tc_id].f.posted;
...
inline void llk_pop_tiles(const std::int32_t dfb_id, const std::int32_t num_tiles) {
    ...
    ckernel::trisc::tile_counters[tc_id].f.acked = num_tiles;
