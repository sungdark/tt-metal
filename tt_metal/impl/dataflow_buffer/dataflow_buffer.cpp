...
void BindDataflowBufferToProducerConsumerKernels(Program& program, uint32_t dfb_id, KernelHandle producer_kernel_handle, KernelHandle consumer_kernel_handle) {
    ...
    if (auto compute_producer = std::dynamic_pointer_cast<ComputeKernel>(producer_kernel)) {
        ...
        dfb->config.producer_risc_mask = 0x100;
    ...
    if (auto compute_consumer = std::dynamic_pointer_cast<ComputeKernel>(consumer_kernel)) {
        ...
        dfb->config.consumer_risc_mask = 0x100;
...
void ProgramImpl::finalize_single_dfb_config(
    ...
    TT_FATAL(config.producer_risc_mask != 0, "producer_risc_mask must have at least one bit set. DFB config should be set a producer_risc_mask or call BindDataflowBufferToProducerConsumerKernels to set the producer and consumer riscs after kernels are created");
    TT_FATAL(config.consumer_risc_mask != 0, "consumer_risc_mask must have at least one bit set. DFB config should be set a consumer_risc_mask or call BindDataflowBufferToProducerConsumerKernels to set the producer and consumer riscs after kernels are created");
